...
class PaymentControllerAuthorizationTest {
    ...
    void getPaymentsByBookingChecksHotelStaffOrAdmin() {
        ...
        when(paymentService.getPaymentsByBookingId(9L, ownerPrincipal)).thenReturn(List.of(payment));
        ...
        verify(paymentService).getPaymentsByBookingId(9L, ownerPrincipal);
    }

    @Test
    void getPaymentsByHotelDelegatesToPermissionAwareService() {
        Payment payment = new Payment();
        PaymentResponse response = PaymentResponse.builder().id(52L).bookingId(11L).build();
        when(paymentService.getPaymentsByHotelId(5L, ownerPrincipal)).thenReturn(List.of(payment));
        when(dtoMapper.toPaymentResponse(payment)).thenReturn(response);

        List<PaymentResponse> result = paymentController.getPaymentsByHotel(5L, ownerPrincipal).getBody();

        assertThat(result).containsExactly(response);
        verify(paymentService).getPaymentsByHotelId(5L, ownerPrincipal);
    }
}
